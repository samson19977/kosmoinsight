...
function Modal({ onClose, onSave, initial }: { onClose: () => void; onSave: (d: Partial<Customer>) => Promise<void>; initial?: Partial<Customer> }) {
  const [form, setForm] = useState({ name: "", phone: "", location: "", region: "Kigali", product_type: "Menstrual Cup", payment_plan: "monthly", notes: "", ...initial });
  ...
  const submit = async (e: FormEvent) => { e.preventDefault(); setLoading(true); try { await onSave(form); onClose(); } catch { setLoading(false); } };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {[["region", "Region", REGIONS], ["product_type", "Product Type", PRODUCTS], ["payment_plan", "Payment Plan", PLANS]].map(([k, label, opts]) => (
            <div key={k}>
              <label style={{ fontSize: 12, fontWeight: 500, color: "#666", display: "block", marginBottom: 5 }}>{label}</label>
              <select style={inp} value={(form as Record<string, unknown>)[k] as string} onChange={set(k)}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 6 }}>
            ...
            <Btn onClick={() => {}}>{loading ? "Saving..." : "Save Customer"}</Btn>
          </div>
        </form>
      </div>
    </div>
...
function CustomerDetail({ customer, onClose, onRefresh }: { customer: Customer; onClose: () => void; onRefresh: () => void }) {
  ...
  useEffect(() => {
    api.customers.payments(customer.id).then((d: unknown) => {
      const data = d as { payments: unknown[] };
  ...
  const runRisk = async () => {
    setLoading(true);
    const pArr = payments as Array<{ status: string }>;
    const missed = pArr.filter(p => p.status === "missed").length;
    const data = await api.predict.risk({ customer_id: customer.id, missed_payments: missed, total_payments: pArr.length || 1, months_active: Math.max(1, Math.floor((Date.now() - new Date(customer.join_date).getTime()) / 86400000 / 30)) });
    setRisk(data as typeof risk);
    setLoading(false);
    onRefresh();
  };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
          {[["Product", customer.product_type || "—"], ["Plan", customer.payment_plan || "—"], ["Location", customer.location || "—"], ["Risk Score", `${(customer.risk_score * 100).toFixed(0)}%`]].map(([l, v]) => (
          ...
          {risk && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", gap: 10 }}>
                {[["Risk Score", `${(risk.risk_score * 100).toFixed(0)}%`, "#EEEDFE", "#3C3489"], ["Risk Level", risk.risk_level, "#FAEEDA", "#633806"], ["Confidence", "85%", "#E1F5EE", "#085041"]].map(([l, v, bg, c]) => (
              ...
              <InsightCard type={risk.risk_level === "high" ? "warning" : risk.risk_level === "medium" ? "info" : "success"} message={risk.recommendation} />
            </div>
          )}
        </div>

        <h4 style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", marginBottom: 10 }}>Payment History ({(payments as unknown[]).length} installments)</h4>
        {payments.length === 0 ? <p style={{ fontSize: 13, color: "#888" }}>No payment records yet.</p> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#FAFAF8" }}>{["#", "Due", "Paid", "Balance", "Status"].map(h => <th key={h} style={{ padding: "8px 10px", textAlign: "left", fontSize: 12, fontWeight: 600, color: "#888" }}>{h}</th>)}</tr></thead>
            <tbody>{(payments as Array<{ installment_number: number; amount_due: number; amount_paid: number; remaining_balance: number; status: string }>).map((p, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #F0EDE6" }}>
                ...
                <td style={{ padding: "8px 10px", color: (p.remaining_balance || 0) > 0 ? "#E24B4A" : "#1D9E75" }}>RWF {(p.remaining_balance || 0).toFixed(0)}</td>
...
export default function CustomersPage() {
  ...
  const load = () => {
    ...
    api.customers.list(params).then(d => { setCustomers(d as Customer[]); setLoading(false); }).catch(() => setLoading(false));
  };
  ...
  return (
    <div>
      <PageHeader title="Customer Management" sub={`${customers.length} customers`} action={<Btn onClick={() => setShowModal(true)}>+ Add Customer</Btn>} />

      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        <input placeholder="Search name, phone, region..." value={search} onChange={e => setSearch(e.target.value)}
                  ...
                  <td style={{ padding: "11px 14px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ width: 44, height: 5, background: "#F0EDE6", borderRadius: 3, overflow: "hidden" }}>
                        <div style={{ width: `${c.risk_score * 100}%`, height: "100%", background: c.risk_level === "high" ? "#E24B4A" : c.risk_level === "medium" ? "#EF9F27" : "#1D9E75", borderRadius: 3 }} />
                      </div>
